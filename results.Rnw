...
@
\caption{Timeline of a) the number of stocks landed by region and assessment status, b) proportion of landed stocks that are assessed, and c) the proportion of landed tonnage derived from assessed stocks. Note that the proportion of landed and assessed stocks by year in this graph is based solely on landings in the NOAA landings database that were resolved to species level and excludes (usually small) landings of higher taxonomic groupings (e.g., stock complexes). Some assessed stocks appear only as stock complexes prior to their first assessment, and we manually added these stocks to our final dataset, but lack a complete time-series of landings in the landings DB (see Methods). The number and proportions of stocks in this graph are therefore under-estimated - see Results text for proportions of currently assessed stocks based on our manually augmented dataset. The dotted vertical line marks the reauthorization of the Sustainable Fisheries Act in 1996 that required rebuilding of overfished stocks, and required biomass limits to be estimated.}
