...
@
\caption{Appropriateness of the Weibull event-time model for the time-to-assessment dataset. If the Weibull applies, data should fall on a line with slope $p$ (the Weibull shape parameter) between the $log(-log(\hat{S}(t)))$, where $\hat{S}(t)$ is the non-parametric Kaplan-Meyer estimate of survival at time $t$, and the log of $t$. Here, $p$ evaluates to \Sexpr{lm_fit[2]}, suggesting an increasing assessment rate with increasing time $t$.}
\end{figure}
...
print(dim(year.table))
print(censored)
print(length(censored))
@
