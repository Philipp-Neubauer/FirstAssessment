...
load('model.outTue Nov 22 01:54:32 2016.rda',v=T)
...
year.table %>% 
  ...
  write.csv(file='dataset.csv',row.names = F)

years.table <- tbl_df(read.csv(file.path(DB,'First year of assessment/v8 Final dataset.csv'),
                              na.strings=c('','NA','#N/A'),
                              stringsAsFactors = F)
)
