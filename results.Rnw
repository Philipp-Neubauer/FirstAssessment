...
harvest rate or quantity above limit levels, generally set at the level that would produce maximum sustainable yield. A target (or limit) harvest quantity can in theory be calculated by combining a target (limit) 
...
\cite[e.g., ][]{helser2006stock}), and agency resources might be fully expended while assessing a small fraction of possible stocks. If the rate of assessing new species is decreasing, this could indicate the need for additional 
...
attributes for US fishes and invertebrates drawn from peer-reviewed stock assessments.  We record the year that each stock landed in the continental US and Alaska (whether caught in US federal or state jurisdictions) first had a 
...
\end{align}
The connection between the event rate and the time-to-event interpretations is made explicit by writing the Weibull density as a function of the product of the rate $r(t)$ at which assessments occur, and the probability $A(t)$ of the assessment not occurring prior to time $t$.
\begin{align}
 f(t) &=  A(t)\times r(t)  \\
