...
#load('model.outFri Mar 10 07:35:33 2017.rda',v=T)
...
year.table %>%
  ...
  mutate(`Mean ex vessel price per kg` = round(price,2),
         ...
         Species = species,
         Stock = stock) %>%
  select(Class, Order,  Family, Species,Stock,Year.of.first.stock.assessment, Region, Habitat, `Mean ex vessel price per kg`, `Maximum landings`, `Fishbase length`,FMP,FMP.name) %>%
...
In addition to confirming many of the added assessments in (2), we found 15 stock assessments meeting our criteria for a formal assessment which were not previously accounted for. Many of these were stocks managed by state agencies so did not appear in the list in (2). These 15 stocks were added to our dataset along with their year of first assessment.
\end{enumerate}

In total, after steps 1--3 we generated a list of 231 U.S. stocks, including 200 stocks with formal stock assessments that qualified under our defined criteria. The remaining 31 stocks did not meet our assessment criteria. They were excluded from our dataset, although after merging our dataset of assessed stocks with the NMFS landings database, those stocks would later re-enter as unassessed stocks. Of these 200 assessed stocks in our full dataset, 188 assessed stocks were used in our final dataset for analysis (Tables \ref{tab:dataset},\ref{tab:dataset_tax}. The remaning 12 assessed stocks (in Table \ref{tab:dataset_missed}) were excluded from our final dataset for analysis because they either they did not have any recorded landings in the NMFS landings database, or the year of first assessment preceded the year in which landings data first began in the NMFS landings database.
