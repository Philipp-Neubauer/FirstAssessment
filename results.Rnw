...
We modeled the scale $\lambda$ of the Weibull distribution as a linear combination of covariates and categorical random effects via a logit-link function:
\begin{align}
...
All variance of each random effect was given a half-Cauchy prior with a scale of $\Theta=100$, regression coefficients had vague normal priors with a precision of $1/\sigma^2 = 1e^{-5}$, and $\tau$ was estimated using a gamma distribution prior with parameters $a=b=1e^{-5}$. 
