...
3.  Stock assessments often require periodic updates (e.g., Pacific hake has been re-assessed annually from X through 2016; \cite[e.g., ][]{helser2006stock}), and agency resources might be fully expended while assessing a small fraction of possible stocks. If the rate of assessing new species  is decreasing, this could indicate the need for additional public resources for stock assessment.  
...
We introduced this study with a common phrase in business management which equally pertains to natural resource management, "what gets measured, gets managed". The National Marine Fisheries Service (NMFS) currently estimates annual catch limits (ACLs) for the vast majority of fishes in federal fisheries management plans, and has established accountability measures that are triggered whenever annual harvest exceeds the ACLs (\cite{methot_implementing_2014}).  We therefore conclude that NMFS is both measuring and managing annual harvest for the majority of US fishes.  However, stock-reduction analyses (SRAs) and other catch-only models (COMs) estimate ACLs without otherwise estimating population size relative to management targets (\cite{dick_depletion-based_2011, wiedenmann_evaluation_2013}), and these SRAs and COMs are used to estimate ACLs for the majority of stocks in most US management regions (\cite{berkson_determination_2015}).  We have excluded SRAs and COMs from our definition of "formal stock assessments", and conclude that NMFS is measuring population abundance only for those species that have an existing stock assessment.  

We acknowledge that it is possible in some cases to rebuild fish stocks by estimating sustainable harvest rates using SRAs or COMs without a formal stock-assessment model (\cite[e.g., ][]{wetzel_model_2011,wetzel_performance_2011}).  Specifically, COMs can be used to develop a harvest plan with fishing at a proportion of the estimated ACL, which is expected to have a pre-specified probability of maintaining population abundance near management targets (\cite{wetzel_performance_2011}). However, we see three main benefits to measuring population abundance (and not just ACLs) for marine fishes:

1.  COMs generally involve managing a fishery to target a constant annual harvest, and this annual harvest is set to perform adequately on average.  Although COMs may perform adequately on average across stocks (some stocks are overfished, others are underfished, but the average stock has appropriate fishing), formal stock assessments are likely to perform adequately on average over time for every individual stock (some decades are overfished, others are underfished, but each individual stock is fished sustainably on average over time).  Both approaches are therefore expected to perform well "on average", but stock assessments have guaruntee of average performance for each individual stock and this guaruntee is important for conservationists and fishers that care about individual stocks rather than stocks in general. 

2.  Furthermore, the ability of formal stock assessments to update harvest plans based on new data has been repeatedly shown to improve management outcomes (\cite[e.g., ][]{carruthers_evaluating_2014}).  For example, managing with a harvest-control rule where fishing mortality is updated based on stock-assessment estimates of population abundance can substantially decrease variability in abundance and fishery catches, even relative to cases where a COM estimates sustainable fishing mortality rates perfectly (\cite{thorson_probability_2015}).  Updating harvest plans based on new data can also prevent cases where stock-reduction analyses estimate a sustainable fishing rate that is too high, which would otherwise collapse the fishery (\cite{wetzel_performance_2011}).  We therefore see benefits both to ocean conservation and fishing industries by continuing to transition stocks managed using COMs to management using formal stock assessments.  

We also note that there are many differences in quality among formal stock assessments.  NMFS categorizes assessments using five "tiers", and high-tier assessments are distinguished by having more or higher-quality data assimilated using a model that has more attention to biological mechanism and realism.  We have ignored these distinctions here, and have instead uses a single cutoff (essentially between statistical population models and SRAs).  We recommend future research that expands our model to include annual probability of transitioning among multiple categories (e.g., among six categories including unassessed and all five NMFS assessment tiers).  This future analysis would therefore allow greater detail regarding changes over time in the average quality of stock assessments, and different regions may come out better or worse when evaluating assessment rates for different assessment types.

Given our operational definition of stock assessments, maximum landings was a particularly strong predictor of the rate at which stocks are first assessed, and ex-vessel price was also positively correlated with assessments. The product of these price and landings variables (i.e., total landed value) is a rough measure of economic value derived from fish and invertebrate stocks. Fisheries managers and scientists must choose among several candidate species in a given region and devote stock assessment time and resources towards only a subset of these. If this choice reflects in part the landed value of stocks, then these results support previous findings (\cite{sethi_global_2010}) suggesting that fishery development is also driven primarily by landed tonnage and ex-vessel prices of fished species.

Results also indicate that flatfishes (Pleuronectiformes), scorpaenfishes (Scorpaeniformes), and groundsharks (Carchariniformes) have a high rate of assessments when controling for price and total landings.  Results for Scorpaeniformes seem reasonable to us, given that this group includes the Pacific rockfishes that have been a topic of conservation concern in Alaska and the US West Coast for (\cite{clark_effect_1993,conway_socioeconomic_2008,myers_maximum_1999}).  Similarly, Carchariniformes have relatively low landings and value, but [SOMETHING TO ADD].  

We note that results from our model could also be used to evaluate and control for systematic differences between assessed and unassessed stocks in the US.  These differences are important, because meta-analysis of assessed stocks is widely used to understand management performance and biological characteristics of marine fishes in general (CITATIONS).  To account for systematic differences between assessed and unassessed stocks, authors could use our model within a "proponsity score matching" framework (citation).  Specifically, our model could be used to predict the probability of being assesed each landed stock in the US.  Stocks with similar "propensity" for stock assessments could then be "matched" in any pairwise comparisons for which an analyst wants to control for the likelihood of being assessed. If analysts then find systematic differences in management outcomes or biological characteristics between assessed and unassessed stocks (e.g., systematic differences in recruitment compensation), then the relationship between assessment propensity and the characteristic could be used to improve predictions for unassessed stocks.  [SOMEONE COULD PLEASE EDIT THIS SECTION] 

Populations in the United States are reaching saturation with respect to the rate of first assessment of stocks. Even though most stocks in all regions are as yet unassessed (Figure \ref{fig:assessed_landed}b), the predicted rate of increase is slow because the stocks most likely to be assessed have already been assessed. However, this pattern is likely not characteristic of most countries. Most will currently have a lower proportion of assessed stocks, and may be more likely still within the period of rapid increase in the proportion of assessed stocks.
