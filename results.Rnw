...
\label{tab:dataset} \\
\centering
Stock &  Assessment year & Region & Habitat & Price (US\$.kg$^{-1}$) & Landings (t) & Length (cm) & SIS cat.\\
\addlinespace
\endfirsthead
