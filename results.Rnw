...
We model the scale $\lambda$ of the Weibull distribution as a function of covariates and categorical variables:
...
\caption[Paramter estimates]{Paramter estimates and the Bayesian probability $P(\theta>0)$ (i.e., the probability that stocks in this category have a higher than average rate of assessment) for any parameter $\theta$. Parameters can be interpreted as the ration of rates (rate effect $\theta$, i.e., rates at which stocks with different characteristics are assessed) or as multiplicative acceleration factors (time effect, i.e., $\nu=\theta\tau^-1=0.5$ suggests a stock with these characteristics is assessed twice as fast as the average stock).} 
\label{tab:fx} \\

Effect & Category & Rate effect ($\theta$) & Time effect ($\nu=\theta\tau^-1$) & $P(\theta>0)$ \\
\addlinespace
\endfirsthead
...
\hline
\addlinespace 
Effect & Category & Rate effect ($\theta$) & Time effect ($\theta\tau^-1$) & $P(\theta>0)$ \\
\addlinespace 
\endhead
