...
\begin{table}[!ht]
\caption[dataset]{Four assessed stocks were excluded from our analysis, due to unavailable ex-vessel prices in the years leading up to the year of first assessment. The assessment year is the year of first stock assessment. The fishery management plan (FMP) name is provided for reference.}
\label{tab:dataset_missed}
\centering
\begin{tabular}{lrlr}
Stock & Assessment year & FMP name & SIS class  \\
<<results="asis">>=
dataset_missed <- read.csv('dataset_missed_SIS.csv',stringsAsFactors = F)
#dataset$Stock <- dataset$Stock)
print(xtable(dataset_missed %>% select(-SIS.name,-FMP) %>% filter(!is.na(as.numeric(Year.of.first.stock.assessment)))), hline.after = -1, only.contents = T, include.colnames = F, include.rownames = F)
@
\end{tabular}
\end{table}
}
...
Because qualifying assessments for these stocks have been accepted and used for management in the past, we treat these as assessed stocks in our analysis and maintain their year of first assessment as noted in Table B.2.
...
II) Of the 368 stocks in our final dataset that were considered unassessed (based on species in the NOAA landings-by-state database that did not pair with assessed stocks) 151 have a corresponding stock in the SIS database. Of these 151 overlapping stocks, 141 (93\%) had an assessment level of 2 or less assigned in the SIS database, and an additional 3 (2\%) had a recent assessment rejected that would have been a level 4 assessment had it not been rejected. The 7 stocks that we classified as unassessed but that have an assessment level of 3 or 4 in the SIS database are:

	\begin{enumerate}
	\item Yelloweye rockfish - Bering Sea / Aleutian Islands; Yelloweye rockfish - Gulf of Alaska
	\item Scamp - Gulf of Mexico; Scamp - Southern Atlantic Coast
	\item White grunt - Southern Atlantic Coast
	\item Golden deepsea crab - Southern Atlantic Coast
	\item Stone crabs (Menippe spp.) - Gulf of Mexico
	\item American lobster - Atlantic Coast [Northeast]
	\item American lobster - Atlantic Coast [Southeast]
	\end{enumerate}
