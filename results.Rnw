...
Some readers might wonder why we focus on formal stock assessments, given that it is possible in some cases to rebuild fish stocks by estimating sustainable harvest rates using stock reduction analysis or other catch-only models (COM)   without a formal stock-assessment model (Wetzel and Punt ref).  Specifically, COMs can be used to develop a harvest plan that is expected to have a pre-specified probability of maintaining population abundance near management targets. Unlike stock assessments, however, COMs generally do not have a mechanism to update this harvest plan because there is no way to incorporate new data.  In a sense, therefore, COMs may perform adequately on average across stocks (some stocks are overfished, others are underfished, but the average stock has appropriate fishing). By contrast, formal stock assessments are likely to perform adequately on average over time for every individual stock (some decades are overfished, others are underfished, but each individual stock is fished sustainably on average over time).  We therefore interpret both approaches as performing adequately "in expectation", but stock assessments have a type of expectation that is important for conservationists and fishers that care about individual stocks rather than stocks in general.  Furthermore, the ability of formal stock assessments to update harvest plans based on new data has been repeatedly shown to improve management outcomes (e.g., Carruthers ref).  For example, managing with a harvest-control rule where fishing mortality is updated based on stock-assessment estimates of population abundance can substantially decrease variability in abundance and fishery catches, even relative to cases where a COM estimates sustainable fishing mortality rates perfectly (Thorson Jensen Hilborn ICESJMS).  Updating harvest plans based on new data can also prevent cases where stock-reduction analyses estimate a sustainable fishing rate that is too high, which would otherwise collapse the fishery (Wetzel and Punt ref).  We therefore see benefits both to ocean conservation and fishing industries by continuing to transition stocks managed using COMs to management using formal stock assessments.  
