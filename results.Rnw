...
We introduced this study with a common phrase in business management which equally pertains to natural resource management, "what gets measured, gets managed". The National Marine Fisheries Service (NMFS) currently estimates annual catch limits (ACLs) for the vast majority of fishes in federal fisheries management plans, and has established accountability measures that are triggered whenever annual harvest exceeds the ACLs (Methot et al. 2012 ICESJMS).  We therefore conclude that NMFS is both measuring and managing annual harvest for the majority of US fishes.  However, stock-reduction analyses (SRAs) and other catch-only models (COMs) estimate ACLs without otherwise estimating population size relative to management targets (Dick and MacCall 2011, Wiedenmann et al. 2013), and these SRAs and COMs are used to estimate ACLs for the majority of stocks in most US management regions (Berkson and Thorson 2015 ICESJMS).  We have excluded SRAs and COMs from our definition of "formal stock assessments", and conclude that NMFS is measuring population abundance only for those species that have an existing stock assessment.  

We acknowledge that it is possible in some cases to rebuild fish stocks by estimating sustainable harvest rates using SRAs or COMs without a formal stock-assessment model (Wetzel and Punt 2011a, 2011b).  Specifically, COMs can be used to develop a harvest plan with fishing at a proportion of the estimated ACL, which is expected to have a pre-specified probability of maintaining population abundance near management targets (Wetzel and Punt 2011 Mar Fresh Res). However, we see three main benefits to measuring population abundance (and not just ACLs) for marine fishes:
...
Given our operational definition of stock assessments, ex-vessel price was a particularly strong predictor of the rate at which stocks are first assessed, followed closely by maximum landings. The product of these price and landings variables (i.e., total landed value) is a rough measure of economic value derived from fish and invertebrate stocks. Fisheries managers and scientists must choose among several candidate species in a given region and devote stock assessment time and resources towards only a subset of these. If this choice reflects in part the landed value of stocks, then these results support previous findings (Sethi et al 2011 PNAS) suggesting that fishery development is also driven primarily by landed tonnage and ex-vessel prices of fished species.

Results also indicate that flatfishes (Pleuronectiformes), scorpaenfishes (Scorpaeniformes), and groundsharks (Carchariniformes) have a high rate of assessments when controling for price and total landings.  Results for Scorpaeniformes seem reasonable to us, given that this group includes the Pacific rockfishes that have been a topic of conservation concern in Alaska and the US West Coast for (Clark 1993, Myers et al. 1999, Conway and Shaw 2008).  Similarly, Carchariniformes have relatively low landings and value, but [SOMETHING TO ADD].  
