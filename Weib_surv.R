model{
  ...
  # linear predictor
  mu[i] <- exp(logmean+betas[1:n.covs] %*% COVS[i,1:n.covs] + region[reg[i]] + habitat[hab[i]] + classfx[class[i]] +  orderfx[order[i]])
  ...
  for (j in 1:n.hab){
    hab.pmu[j] <- exp(logmean+ habitat[j] + classfx.pred + orderfx.pred  + regfx.pred)
  ...
  for (l in 1:n.class){
    class.pmu[l] <- exp(logmean+ habfx.pred + classfx[l] + orderfx.pred + regfx.pred)
  ...
  for (l in 1:n.order){
    order.pmu[l] <- exp(logmean+ habfx.pred + classfx[classord[l]] + orderfx[l] + regfx.pred)
  ...
  for (l in 1:n.region){
    region.pmu[l] <- exp(logmean+ habfx.pred + classfx.pred + region[l] + orderfx.pred )
