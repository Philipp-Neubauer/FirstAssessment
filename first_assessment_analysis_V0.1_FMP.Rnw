...
land.price$Species[grepl('SHARK, SPINY DOGFISH',land.price$Species,ignore.case = T) | grepl('SHARK, DOGFISH',land.price$Species,ignore.case = T)] <- 'SHARK, SPINY DOGFISH'
...
land.price$Species[grepl('POMPANO, FLORIDA',land.price$Species,ignore.case = T) & land.price$region == 'USWC-48'] <- 'POMPANO, PACIFIC'

land.price <- land.price %>% filter(!(grepl('LINGCOD',land.price$Species,ignore.case = T) &  grepl('USEC',land.price$region)))
...
stock.landings$region[grepl('croaker',stock.landings$stock)] <- 'USEC-NE'
stock.landings$region[grepl('weakfish',stock.landings$stock)] <- 'USEC-NE'
stock.landings$region[grepl('scup',stock.landings$stock)] <- 'USEC-NE'
stock.landings$region[grepl('Homarus',stock.landings$species)] <- 'USEC-NE'
stock.landings$region[grepl('USEC smooth dogfish shark',stock.landings$stock)] <- 'USEC-NE'
stock.landings$region[grepl('USNE midAtl red drum',stock.landings$stock)] <- 'USEC-NE'
...
manual_landprice <- read.csv(file.path(DB,'First year of assessment/manual_max_landings_price.csv'),header=T,stringsAsFactors = F)
...
year.table[which(year.table$stock=="USWC China rockfish (northern)"),'minyear'] <- mean(year.table[grep("USWC China rockfish",year.table$stock),]$minyear,na.rm=T)

year.table[grep("anchovy",year.table$stock),'minyear'] <- 1960
year.table[grep("Kamchat",year.table$stock),'minyear'] <- 1960

year.table <- year.table %>% filter(maxyear>1960, !is.na(minyear), !is.na(price))
...
db <- taxize::classification(data.frame(year.table[is.na(year.table$Order),'species'])$species, db='ncbi')
...
year.table[is.na(year.table$Order),c('Class','Order','Family')] <- rbind(c("Bivalvia","Veneroida","Corbiculidae"),c("Bivalvia","Ostreoida","Ostreidae"))
...
year.table$mainregion[grep('USEC-SE',year.table$mainregion)] <- 'USEC-SE'
...
year.tables <- read.csv('dataset_v12_FMP.csv')
fmp <- year.tables %>% select(FMP,stock)
year.table <- inner_join(year.table %>% select(-FMP),fmp) %>% mutate(FMP = as.numeric(FMP),
                                                    FMP = ifelse(FMP>1,1,FMP))

year.table <- unique(year.table)
dim(year.table)
#assessment time
...
require(rstanarm)
fmp_reg <- stan_glmer(FMP ~ std_log10_length + 
           std_log10_price + 
           std_log10_land + 
           std_log10_landprice + 
           (1|habitat_FB.SLB) + 
         ...
         prior = student_t(df = 100), 
         prior_intercept = student_t(df = 100),
         chains = 2, cores = 2)

ci95 <- posterior_interval(fmp_reg, prob = 0.95)
round(ci95, 2)
...
COVS <- year.table %>%
  select(std_log10_length,
         std_log10_price,
         std_log10_land,
         std_log10_landprice,FMP) %>% as.matrix()
