...
stock.landings.sum <- stock.landings.sum %>%
  filter(!(nstocks>1 & max.land<0.05*nmland),max.land>0.1) %>% 
  ungroup()
...
year.table$lengths[is.na(year.table$lengths)] <- c(NA,13.3,17,NA,NA,40,17,NA,NA,10,8,8)
