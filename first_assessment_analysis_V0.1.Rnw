...
stock.landings.sum.model <- stock.landings.sum.model %>%
  filter(!(nstocks>1 & max.land<0.05*nmland & !(stock %in% years.table$Stock.name)),max.land>0.1) %>% 
  ungroup()
...
full.tab.model <- left_join(stock.landings.sum.model,
                      years.table,
                      by=c('stock' = 'Stock.name')) %>%
  mutate(species = ifelse(is.na(Latin.name_assessment), species, Latin.name_assessment))
...
year.table <- filter(year.table,!is.na(species),grepl(' ',species)) %>%
...
year.table$a.time <- as.numeric(year.table$Year.of.first.stock.assessment) - devtime
...
ctime <- a.time+1
ctime[is.na(a.time)] <- year.table$maxyear[is.na(a.time)]-year.table$minyear[is.na(a.time)]+1
