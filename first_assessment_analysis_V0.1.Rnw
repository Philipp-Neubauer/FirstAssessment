...
stock.landings.sum <- tbl_df(stock.landings) %>%
  ...
  mutate(cum.land = max(total_landings),
         price = mean(price,na.rm=T)) %>%
  ungroup()

save(stock.landings.sum,file='stock.landings.region.rda')
...
no_match <- year.table$species[is.na(fishmatch) & grepl(' ',year.table$species)]
...
fishmatch[is.na(fishmatch) & grepl(' ',year.table$species)] <- repl_specs
...
inv_syn <- lapply(no_match,rfishbase::synonyms, limit=50, server='http://fishbase.ropensci.org/sealifebase')
...
year.table[is.na(fishmatch) & is.na(year.table$habitat_MM), 'habitat_MM'] <- inv_habitat[is.na(year.table$habitat_MM[is.na(fishmatch)])]

year.table[is.na(year.table$habitat_MM), c('stock','species')] %>% data.frame()
...
year.table$habitat_MM[grepl('bathy', year.table$habitat_MM)] <- 'bathy-'
...
year.table$habitat_MM[grepl('sessile', year.table$habitat_MM)] <- 'reef'
...
year.table <- year.table %>% 
  filter(!is.na(habitat_MM), !is.na(species),!is.na(Order))  %>%
  mutate(lengths = ifelse(is.na(lengths),Lmax..cm.,lengths)) %>%
  as.data.frame()

year.table$lengths[is.na(year.table$lengths)] <- c(35,13.3,NA,12.9,8)
