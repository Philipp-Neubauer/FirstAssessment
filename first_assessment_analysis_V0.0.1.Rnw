...
year.table <- tbl_df(read.csv('~/Work/Dropbox/First year of assessment/V3_Final_dataset.csv',na.strings=c('','NA')))
...
The contrast in the status is with respect to the base case (never been in a rebuiling plan). The model doesn't explain much ($<8\%$) and the fit isn't great either:
