...
u.spec <- unique(year.table$species)

# FISH
# match species to fishbase entries to get habitat. 

specs <- paste(fishbase$Genus,fishbase$Species)
# parallelise this call
sp_match <- parallel::mclapply(u.spec, function(name) {
  unique(c(which(specs %in% name),which(fishbase$Genus %in% name),which(fishbase$Family %in% name)))
}, mc.cores=3)

class <- lapply(sp_match, function(x) unique(fishbase[x,'Class']))

unknown.habitat.spec <- u.spec[u.spec %in% unique(year.table$species[is.na(year.table$habitat_MM)])]

smatch <- sp_match[u.spec %in% unknown.habitat.spec]

sps <- specs[unlist(smatch)]

habref  <- lapply(1:length(smatch),function(s) {
  if(length(smatch[[s]])>0) return(data.frame(smatch[[s]],s))
})

habref <- do.call('rbind',habref)
...
habitat <- tapply(habitats[habref[,1]], habref[,2], function(x) names(which.max(table(x))))

mix <- match(year.table$species,unknown.habitat.spec)[is.na(year.table$habitat_MM)]

year.table$habitat_MM[is.na(year.table$habitat_MM)] <- habitat[match(mix,as.numeric(names(habitat)))]

### inverts
sealifebase <- load_taxa(server=SEALIFEBASE_API,update = T,limit=120000)

unknown.inv.spec <- u.spec[u.spec %in% unique(year.table$species[is.na(year.table$habitat_MM)])]

invspecs <- paste(sealifebase$Genus,sealifebase$Species)

invmatch <- lapply(unknown.inv.spec, function(name) {
  unique(c(which(invspecs %in% name),which(sealifebase$Genus %in% name),which(sealifebase$Family %in% name)))
})

inv_habitats <- species(fields='DemersPelag', server='http://fishbase.ropensci.org/sealifebase',limit=120000)

SC <- inv_habitats[match(inv_habitats$SpecCode, sealifebase$SpecCode),]



habref  <- lapply(1:length(invmatch),function(s) {
  if(length(invmatch[[s]])>0) return(data.frame(invmatch[[s]],s))
})

habref <- do.call('rbind',habref)
 

inv_habitat <- unlist(tapply(inv_habitats[habref[,1],'DemersPelag'], habref[,2], function(x) names(which.max(table(x)))))

mix <- match(year.table$species,unknown.inv.spec)[is.na(year.table$habitat_MM)]

year.table$habitat_MM[is.na(year.table$habitat_MM)] <- inv_habitat[match(mix,as.numeric(names(inv_habitat)))]
...
# get a taxon variable for all species. 
year.table <- year.table %>% filter(!is.na(habitat_MM), !is.na(species),region!='inland') %>% as.data.frame()
      
fls <- fishbase[match(year.table$species,specs),'Order'] 
flf <- fishbase[match(year.table$species,fishbase$Family),'Order'] 
flg <- fishbase[match(year.table$species,fishbase$Genus),'Order'] 


sls <- sealifebase[match(year.table$species,invspecs),'Order']
slf <- sealifebase[match(year.table$species,sealifebase$Family),'Order']

all <- cbind(fls,flf,sls,slf)

year.table$order

year.table$order[is.na(year.table$order)] <- sealifebase[match(year.table$species[is.na(year.table$order)],invspecs),'Order'] %>% as.data.frame()
...
ggplot(year.table) + geom_bar(aes(fill=assessed,x=habitat_MM)) +coord_flip() + theme_bw()
...
formula=inla.surv(a.time,!censored) ~ land+price+region+f(habitat_MM,model='iid') + f(order,model='iid')
model=inla(formula, family ="weibull", data=year.table)
...
a.out <- coda.samples(JM,variable.names=c('betas','habitat','fp.sd.tax','fp.sd.hab','taxonomy','CS'),n.iter = 2e3, thin = 10)
