...
full.tab %>% 
  ...
  summarise(ns = n()) %>%
  ggplot() + geom_line(aes(col=assessed,x=year,y=ns,linetype = region)) + theme_bw()
@
...
ggplot(year.table) + geom_bar(aes(fill=assessed,x=habitat,y=(..count..)/sum(..count..))) + coord_flip() + theme_bw()

ggplot(year.table) + geom_bar(aes(fill=assessed,x=phylum,y=(..count..)/sum(..count..))) + coord_flip() + theme_bw()

ggplot(year.table %>% filter(phylum == 'Chordata')) + 
  geom_bar(aes(fill=assessed,x=order,y=(..count..)/sum(..count..))) + coord_flip()
...
a.out <- coda.samples(JM,variable.names=c('betas','habitat','fp.var','CS'),n.iter = 2e5, thin = 100)
...
CS.means <- CS.full %>%
  group_by(Parameter) %>%
  summarise(post.mean = median(MCMC),
...
# intervals
km.cs.h <- survfit(Surv(CS_high,!censored) ~ 1)
summary.km.cs.h <- summary(km.cs.h)
rcu.h <- summary.km.cs.h$time # Cox-Snell residuals of
                            # uncensored points.
surv.cs.h <- summary.km.cs.h$surv
km.cs.l <- survfit(Surv(CS_low,!censored) ~ 1)
summary.km.cs.l <- summary(km.cs.l)
rcu.l <- summary.km.cs.l$time # Cox-Snell residuals of
                            # uncensored points.
surv.cs.l <- summary.km.cs.l$surv

plot(rcu,-log(surv.cs),type="p",pch=16,
xlab="Cox-Snell residual",ylab="Cumulative hazard")
lines(rcu.l,-log(surv.cs.l),lty=2)
lines(rcu.h,-log(surv.cs.h),lty=2)
abline(a=0,b=1,col=3,lwd=2); 

@

It looks as though the fit of the Weibull isn't too bad, some deviation is expected in the tails of the distribution, but over the bulk it seems to follow the 1:1 line fairly closely. We could also plot the posterior for the CS residuals and see if the 1:1 line falls outside a 95\% interval for any points.
