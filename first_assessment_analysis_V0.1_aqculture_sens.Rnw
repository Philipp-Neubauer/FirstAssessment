...
year.table <- year.table %>% filter(!Family %in% c('Ostreidae','Mytilidae','Veneridae'))
...
year.table <- inner_join(year.table,fmp) %>% mutate(FMP = as.numeric(FMP)-1,
...
save.image(file = 'model_inputs_aqsensitivity.Rdata')
